...
function index(req, res) {
  UserService.index().then(function (users) {
    res.json(200, users);
  }).catch(function (err) {
    res.send(500, err);
  });
...
function create(req, res, next) {
  UserService.create(req.body).then(function (token) {
...
function show(req, res, next) {
  UserService.show(req.params.id).then(function (profile) {
...
function destroy(req, res) {
  UserService.destroy(req.params.id).then(function () {
    res.send(204);
  }).catch(function (err) {
...
function changePassword(req, res, next) {
  ...
  UserService.changePassword(userId, oldPass, newPass).then(function () {
    res.send(200);
  }).catch(function (err) {
...
function me(req, res, next) {
  UserService.me(req.user._id).then(function (user) {
    res.json(user);
  }).catch(function (err) {
    if (err && err.code && err.code === 'USER_NOT_FOUND') {
      return res.send(401);
...
function updateScore(req, res, next) {
  ...
  UserService.updateScore(req.params.id, req.body.score).then(function (affectedNumber) {
